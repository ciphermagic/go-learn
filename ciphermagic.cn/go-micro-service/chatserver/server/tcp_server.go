...
func (s *TcpServer) accept(conn net.Conn) *client {
	...
	client := &client{
		conn:   conn,
		writer: protocol2.NewWriter(conn),
	}
...
func (s *TcpServer) serve(client *client) {
	cmdReader := protocol2.NewReader(client.conn)
	...
	for {
		...
		if cmd != nil {
			switch v := cmd.(type) {
			case protocol2.SendCmd:
				go s.Broadcast(protocol2.MessCmd{
					Message: v.Message,
					Name:    client.name,
				})
			case protocol2.NameCmd:
				client.name = v.Name
			}
		}
